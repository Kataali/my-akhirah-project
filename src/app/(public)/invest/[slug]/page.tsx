...
export default function InvestPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: camp }, { data: { user: u } }] = await Promise.all([
        supabase.from("campaigns").select("id, slug, title, target_amount, raised_amount, location").eq("slug", params.slug).single(),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!user) return router.push(`/auth/login?redirect=/invest/${params.slug}`);
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12">
      <Link href={`/campaigns/${campaign.slug}`} className="btn-ghost mb-6 -ml-2 text-earth-500">
        <ArrowLeft size={16} /> Back to campaign
      </Link>

      <div className="card p-8">
        <h1 className="font-display text-2xl font-bold text-earth-900 mb-1">Make a contribution</h1>
        <p className="text-earth-500 text-sm mb-6">{campaign.title} · {campaign.location}</p>

        {!user && (
          <div className="mb-6 rounded-lg bg-earth-50 border border-earth-200 p-4 text-sm text-earth-600">
            <Link href={`/auth/login?redirect=/invest/${campaign.slug}`} className="text-earth-700 font-semibold underline">Sign in</Link>{" "}
            to contribute and track your impact history.
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="text-sm text-earth-600">Contribute anonymously</span>
          </label>
          ...
          >
            ...
            {loading ? "Redirecting to payment…" : `Contribute ${amount ? formatCurrency(parseFloat(amount)) : ""}`}
          </button>

          <p className="flex items-center justify-center gap-1.5 text-xs text-earth-400">
            <Lock size={12} /> Secure payment via Paystack · Card or Mobile Money
          </p>
        </form>
      </div>
    </div>
