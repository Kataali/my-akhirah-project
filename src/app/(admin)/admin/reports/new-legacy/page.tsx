...
async function createLegacyReport(formData: FormData) {
  ...
  if (!campaign_title) throw new Error("Campaign title is required for legacy reports.");
...
export default function NewLegacyReportPage() {
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        <h1 className="font-display text-2xl font-bold text-earth-900">New Legacy Impact Report</h1>
        <p className="text-earth-500 text-sm mt-1">Create a report for a campaign that isn't in the site campaigns list.</p>
      </div>

      <form action={createLegacyReport} className="card p-7 space-y-5">
        <div>
          <label className="label">Campaign title *</label>
          <input name="campaign_title" required className="input" placeholder="e.g. Flood relief at Kpandai - June 2023" />
        </div>
        ...
        <div>
          ...
          <div>
            ...
            <input name="event_date" type="date" required className="input" defaultValue={new Date().toISOString().slice(0,10)} />
            <p className="text-xs text-earth-400 mt-1">Select the date the activity/donation occurred.</p>
          </div>
        ...
        <div>
          ...
          <p className="text-xs text-earth-400 mt-1">Upload photos to Supabase Storage first, then paste public URLs here — use newlines or commas to separate multiple URLs.</p>
        </div>
        ...
        <div>
          ...
          <textarea name="items_delivered" rows={4} className="input resize-none font-mono text-xs"
            defaultValue={JSON.stringify([{ name: "Example item", quantity: 50, unit: "units" }], null, 2)}
          />
